...
import { awardedPpForProgress, calculateTotalPp } from '@/services/ScoringEngine';
...
> {
  ...
  const siblings = await prisma.record.findMany({
    ...
    const approvedSiblings = siblings.filter((s) => s.status === RecordStatus.APPROVED);
    for (const old of approvedSiblings) {
    ...
    const approvedIds = approvedSiblings.map((s) => s.id);
