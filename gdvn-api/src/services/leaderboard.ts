...
import { pickDecoAndLayoutBadges, cpFromVnLevels } from '@/services/creatorPoints';
import { calculateModePp, pickHardestLevel } from '@/services/recordUtils';
...
export async function getCachedLeaderboard(mode: string) {
  return await getCached<any>(`lb:${mode}`);
}

export async function setCachedLeaderboard(mode: string, body: unknown) {
  await setCache(`lb:${mode}`, body, CACHE_TTL);
}

export async function clearLeaderboardCache() {
  await invalidateCache('lb:*');
}
