(function (root) {
  ...
  function discordToFaqHtml(src) {
    ...
    src = src.replace(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g, function (_, text, url) {
      return protect('<a href="' + escapeAttr(url) + '">') + text + protect('</a>');
    });
    ...
    while (i < lines.length) {
      ...
      var matchDetails = line.match(/^:::\s+(.+)$/);
      if (matchDetails) {
        var title = matchDetails[1];
        var content = [];
        i += 1;
        ...
        var innerHtml = discordToFaqHtml(content.join('\n'));
        blocks.push('<details><summary>' + escapeHtml(title) + '</summary>\n' + innerHtml + '\n</details>');
        continue;
      }
  ...
  function faqHtmlToDiscord(html) {
    if (!html) return '';
    let md = String(html);
    
    // Reverse iframe embeds to plain URLs
    md = md.replace(/<iframe[^>]*src="([^"]+)"[^>]*><\/iframe>/gi, function(_, src) {
      if (src.includes('youtube.com/embed/')) {
        return src.replace('youtube.com/embed/', 'youtube.com/watch?v=');
      }
      if (src.includes('player.vimeo.com/video/')) {
        return src.replace('player.vimeo.com/video/', 'vimeo.com/');
      }
      if (src.includes('tiktok.com/embed/v2/')) {
        return src.replace('tiktok.com/embed/v2/', 'tiktok.com/@u/video/');
      }
    ...
    md = md.replace(/<u><strong><em>(.*?)<\/em><\/strong><\/u>/gi, '__***$1***__');
    md = md.replace(/<u><strong>(.*?)<\/strong><\/u>/gi, '__**$1**__');
    md = md.replace(/<u><em>(.*?)<\/em><\/u>/gi, '__*$1*__');
    ...
    md = md.replace(/<a[^>]*href="([^"]+)"[^>]*>(.*?)<\/a>/gi, function(_, url, text) {
    ...
    md = md.replace(/<small>(.*?)<\/small>/gi, '-# $1\n');

    md = md.replace(/<blockquote>([\s\S]*?)<\/blockquote>/gi, function(_, quote) {
      return quote.trim().split('\n').map(l => '> ' + l).join('\n') + '\n\n';
    });

    // Lists (Basic handle)
    md = md.replace(/<ul>([\s\S]*?)<\/ul>/gi, function(_, list) {
      return list.replace(/<li>(.*?)<\/li>/gi, '- $1\n') + '\n';
    });
    md = md.replace(/<ol>([\s\S]*?)<\/ol>/gi, function(_, list) {
      let i = 1;
      return list.replace(/<li>(.*?)<\/li>/gi, () => `${i++}. $1\n`) + '\n';
    });
    
    md = md.replace(/<details>\s*<summary>(.*?)<\/summary>\n?([\s\S]*?)<\/details>/gi, function(_, title, content) {
      return '::: ' + title + '\n' + content.trim() + '\n:::\n\n';
    });

    // Cleanup extra newlines and decode HTML entities
    md = md.replace(/\n{3,}/g, '\n\n');
    md = md.replace(/&lt;/g, '<')
           .replace(/&gt;/g, '>')
           .replace(/&amp;/g, '&')
  ...
  root.faqHtmlToDiscord = faqHtmlToDiscord;
