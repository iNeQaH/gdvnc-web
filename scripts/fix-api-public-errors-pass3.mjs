...
for (const file of files) {
  ...
  const orig = src;
  src = src.replace(
    /return NextResponse\.json\(\{\s*error:\s*error\.message\s*\|\|\s*('(?:\\'|[^'])*'),\s*500\s*\}\s*\);/g,
    'return publicApiError(error, $1, 500);'
  );
  src = src.replace(
    /return NextResponse\.json\(\s*\{\s*error:\s*error\.message\s*\|\|\s*('(?:\\'|[^'])*'),\s*500\s*\}\s*\);/g,
    'return publicApiError(error, $1, 500);'
  );
  src = src.replace(
    /return NextResponse\.json\(\s*\n\s*\{\s*error:\s*error\.message\s*\|\|\s*('(?:\\'|[^'])*'),\s*500\s*\}\s*\);/g,
    'return publicApiError(error, $1, 500);'
  );
  if (src !== orig) {
    ...
    console.log('pass3', path.relative(process.cwd(), file));
  }
}
